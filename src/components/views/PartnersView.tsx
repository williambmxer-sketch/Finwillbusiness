...
export function PartnersView() {
  ...
  return (
    <div className="mx-auto w-full max-w-7xl px-4 py-6 lg:px-8">
      <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><div className="mb-3 inline-flex items-center gap-2 rounded-full bg-violet-500/10 px-3 py-1.5 text-[10px] font-black uppercase tracking-widest text-violet-600"><HandCoins className="h-3.5 w-3.5" /> Sócios e titular</div><h1 className="text-2xl font-black tracking-tight md:text-3xl">Pró-labore, retiradas e aportes</h1><p className="mt-1 text-sm text-muted-foreground">Movimentos dos sócios separados das receitas e despesas do negócio.</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => openPreset('prolabore')} className="rounded-xl bg-primary px-3.5 py-2.5 text-xs font-bold text-primary-foreground">Registrar pró-labore</button><button type="button" onClick={() => openPreset('withdrawal')} className="rounded-xl border border-border bg-card px-3.5 py-2.5 text-xs font-bold">Retirada extra</button><button type="button" onClick={() => openPreset('contribution')} className="rounded-xl border border-border bg-card px-3.5 py-2.5 text-xs font-bold">Aporte</button></div></div>
